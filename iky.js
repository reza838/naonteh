...
await ikyy.updatePresence(from, Presence.recording)
} else if (autoketik) {
...
ikyy.sendMessage(from,{url:'./'+kyyyy},audio,{mimetype:'audio/mpeg'})
  ...
  case 'xnxxsearch':
      ...
      case 'tiktokaudio':
             ...
             ikyy.sendMessage(from, get_audio, document, { mimetype: Mimetype.mp4Audio, quoted: freply })
             break
      case 'fbdl':
      case 'fb':
      case 'facebok':
...
//------------------< Owner >-------------------
      ...
      case 'broadcast':
             ...
             for (let _ of anu) {
             ikyy.sendMessage(_.jid, bc, image, {quoted:freply,caption: `*「 Bilaw-Bot NEWS 」*\n\n${body.slice(4)}`})
}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu) {
             sendMess(_.jid, `*「 Bilaw-Bot NEWS 」*\n\n${body.slice(4)}`)
}
             reply('Suksess broadcast')
}
             break
...
case 'caripesan':  //by ANU TEAM
       ...
       case 'bugreport':
              ...
              ikyy.sendMessage('6283103242015@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'readall':
...
if (budy.includes("Numa")){
  ...
  if (budy.includes("Assalamualaikum")){
		...
		const loli = fs.readFileSync('./assets/salam.mp3')
